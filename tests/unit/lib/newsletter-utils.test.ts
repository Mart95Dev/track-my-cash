...
describe("generateUnsubscribeUrl (TU-157-1)", () => {
  it("TU-157-1 — génère une URL avec email et token HMAC", () => {
    const url = generateUnsubscribeUrl("test@example.com", "https://trackmycash.com");
  ...
  it("génère des tokens différents pour des emails différents", () => {
    const url1 = generateUnsubscribeUrl("a@test.com", "https://trackmycash.com");
    const url2 = generateUnsubscribeUrl("b@test.com", "https://trackmycash.com");
  ...
  it("génère le même token pour le même email (déterministe)", () => {
    const url1 = generateUnsubscribeUrl("same@test.com", "https://trackmycash.com");
    const url2 = generateUnsubscribeUrl("same@test.com", "https://trackmycash.com");
...
describe("verifyUnsubscribeToken (TU-157-2, TU-157-3, TU-157-4)", () => {
  it("TU-157-2 — retourne true pour un token valide", () => {
    const url = generateUnsubscribeUrl("valid@test.com", "https://trackmycash.com");
  ...
  it("TU-157-4 — retourne false si email modifié (token d'un autre email)", () => {
    const url = generateUnsubscribeUrl("original@test.com", "https://trackmycash.com");
    const token = new URL(url).searchParams.get("token")!;
...
describe("QA-157 — generateUnsubscribeUrl edge cases", () => {
  ...
  it("QA-157-2 — l'URL générée est une URL valide parseable", () => {
    const url = generateUnsubscribeUrl("special+char@test.com", "https://trackmycash.com");
  ...
  it("QA-157-3 — email avec caractères spéciaux est correctement encodé dans l'URL", () => {
    const url = generateUnsubscribeUrl("user+tag@test.com", "https://trackmycash.com");
