...
describe("Banque Populaire parser — canHandle", () => {
  ...
  it("TU-1-2 : canHandle retourne false si content est undefined (ex: XLSX)", () => {
    expect(banquePopulaireParser.canHandle("releve.xlsx", undefined)).toBe(false);
  });
});

describe("Banque Populaire parser — parse", () => {
  it("TU-1-3 : parse détecte le solde initial (1500.25) depuis 'Solde'", () => {
    const result = banquePopulaireParser.parse(FIXTURE_CONTENT, null) as ParseResult;
